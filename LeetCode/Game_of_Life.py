        ...
        for c in range(cols):
            ...
            for dr, dc in adjs:
                if 0 <= r + dr < rows and 0 <= c + dc < cols \
                        and board[r + dr][c + dc] in (1, 3):
                    adj_1s += 1

            # We use 2 for a zero changing to a 1
            # and 3 for a 1 changing to a 0
            if board[r][c] == 1 and (adj_1s < 2 or adj_1s > 3):
                board[r][c] = 3
            elif board[r][c] == 0 and adj_1s == 3:
                board[r][c] = 2
